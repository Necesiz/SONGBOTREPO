import os

class Config:
    BOT_TOKEN = os.environ.get("5681957815:AAGzVJmtl_LH7Vs-MfgbcjP9Hh-y0wHJrb0")
    API_ID = int(os.environ.get("17568815"))
    API_HASH = os.environ.get("177622d39f23e7c3d015f3d6ebaacd31")
    BOT_OWNER = os.environ.get("abasof_015")
    BOT_USERNAME = os.environ.get("AzeSongRobot")
    PLAYLIST_NAME = os.environ.get("azesongplaylist")
    GROUP = os.environ.get("GROUP") 
    PLAYLIST_ID = int(os.environ.get("-1001877177311"))
